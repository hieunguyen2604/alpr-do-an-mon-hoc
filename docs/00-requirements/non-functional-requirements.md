...
| Chính xác ký tự (A4) | ≥ 0.92 | 🟡 **0,9483** — đạt ngưỡng tối thiểu, dưới mục tiêu 0,95. Lật từ ❌ 0,8848 (đo 20/07) nhờ các bản sửa đọc biển 2 dòng và bậc thang thử-lại ([27](../reports/27-retry-ladder-cost-benefit.md)) |
| Chuỗi đúng sau hậu xử lý (A6) | ≥ 0.85 | ❌ **0,7701** (Chương 5 mục 5.5.2–5.5.3; bảng sửa ký tự dựng từ số liệu nhầm lẫn đo được, [41](../reports/41-measured-confusion-tables.md)). Toàn bộ khoảng cách nằm ở biển 2 dòng: biển 1 dòng đạt **0,9541**, biển 2 dòng **0,7234** — chênh **23,07 điểm**. Hậu xử lý đóng góp **+13,28 điểm** (sửa đúng **372** biển) và không phá hỏng biển nào |
| Độ chính xác E2E (A7) | ≥ 0.88 | ❌ **56,3%** *(KTC 95% [52,0 ; 60,7])* — đo ở **mức ảnh toàn cảnh**, nhãn *"không đại diện"* của giao thức cũ đã được gỡ ([34](../reports/34-scene-level-a7.md); quyển: mục 5.5.5, Bảng 5.8). ⚠ **Hạn chế phải nói kèm: nhãn do một mô hình ngôn ngữ-thị giác đọc, không phải người** — độc lập với cả bốn cấu hình được so sánh nên không vòng tròn, nhưng vẫn có thể sai theo cách riêng của nó. Ước lượng phân tầng trên 1.606 khung biển của 1.514 ảnh hiện trường, 608 khung có nhãn chuỗi (tầng dễ đếm hết 372/374): tầng dễ 96,8% · tầng khó 44,1%. Con số cũ **0,5552** đo trên ảnh *đã cắt sẵn* nên không đại diện; hai phép đo gần nhau **một cách tình cờ** — chúng đo hai thứ khác nhau trên hai ngữ liệu khác nhau |
| Độ trễ 1 ảnh (p95, CPU) | ≤ 800 ms | ✅ **509,76 ms** in-process — đạt sàn 1.500 ms, trên mục tiêu 800 ms. Bậc thang thử-lại là nguyên nhân (chỉ chạy khi đọc hỏng: p50 chỉ 405,8 ms). Tắt siêu phân giải mặc định đã kéo p95 từ 1.514 xuống 1.143 ms ([27](../reports/27-retry-ladder-cost-benefit.md) mục 5) |
| Webcam FPS (CPU) | ≥ 5 | ✅ **5,63 FPS hiệu dụng** — vượt **cả mục tiêu 5 FPS** ([38](../reports/38-runtime-backend-and-nfr-p2.md)). Con số **2,379** công bố trước đó đo khi máy đang cõng ~560% CPU của tiến trình khác và harness **đã tự dán nhãn bi quan**; nó không tái lập được ở bất kỳ điều kiện nào trong 6 lần đo lại. Đo ở tầng API `POST /api/detect/frame` qua HTTP loopback, hàng đợi **một khe** đúng như giao diện dùng: trong 60,5 giây camera ảo 30 FPS chào 1.815 khung, gửi được **144**, bỏ **1.671**, **0 lỗi**. p50 180,1 ms nhưng p95 1.247,7 ms — chính bậc thang thử-lại kéo dài đuôi, và ở chế độ một khe thì đuôi quyết định thông lượng. Con số này là **cận trên**: nó chưa tính thời gian camera thu hình, mã hoá JPEG phía trình duyệt và vẽ canvas |
| Tốc độ xử lý video (P3) | ≥ 0.3× | ✅ **0,746× thời gian thực** — vượt xa mục tiêu (đo 02/08/2026). Video 14,25 s xử lý hết 19,1 s đầu-cuối kể cả tải lên và độ hạt polling 0,25 s. Phân tích 1 khung trên mỗi 5 khung, **0,546 s/khung** |
| Chạy liên tục (R4) | ≥ 99% | ✅ **100,0%** — 5.337 yêu cầu, **0 lỗi**, RSS phẳng 0,726→0,810 GB (không rò rỉ bộ nhớ). ⚠ Chạy **15 phút**, đặc tả ghi 60 phút: đây là mẫu con, phải nói rõ khi bảo vệ |
| CSDL sống sót khởi động lại (R5) | 100% | ✅ **0/7.977 bản ghi mất** — đếm và 10 mẫu đối chiếu đều trùng khớp sau khi giết và khởi động lại tiến trình. Đo bằng **uvicorn thật**, không dùng `TestClient`: client trong tiến trình sẽ vô hiệu hoá đúng thứ cần đo |
| Bao phủ test | ≥ 70% | ✅ **1.004 test thu thập / 1.004 đạt, 0 `xfail`, 0 fail**. Bao phủ tầng nghiệp vụ: **87,7%** (đo 20/07/2026, [13-refactor-result.json](../reports/13-refactor-result.json)); **88,1%** ở lần đo Phase 7 trước đó ([07-testing-report.md](../reports/07-testing-report.md) mục 3.3). Cả hai mốc đều đạt |
