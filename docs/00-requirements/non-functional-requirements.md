...
| Chạy liên tục (R4) | ≥ 99% | ✅ **100,0%** — 2.028 yêu cầu, **0 lỗi**, RSS phẳng 0,726→0,810 GB (không rò rỉ bộ nhớ). ⚠ Chạy **15 phút**, đặc tả ghi 60 phút: đây là mẫu con, phải nói rõ khi bảo vệ |
| CSDL sống sót khởi động lại (R5) | 100% | ✅ **0/9.031 bản ghi mất** — đếm và 10 mẫu đối chiếu đều trùng khớp sau khi giết và khởi động lại tiến trình. Đo bằng **uvicorn thật**, không dùng `TestClient`: client trong tiến trình sẽ vô hiệu hoá đúng thứ cần đo |
| Bao phủ test | ≥ 70% | ✅ **1.002 test thu thập / 1.002 đạt, 0 `xfail`, 0 fail**. Bao phủ tầng nghiệp vụ: **87,7%** (đo 20/07/2026, [13-refactor-result.json](../reports/13-refactor-result.json)); **88,1%** ở lần đo Phase 7 trước đó ([07-testing-report.md](../reports/07-testing-report.md) mục 3.3). Cả hai mốc đều đạt |
