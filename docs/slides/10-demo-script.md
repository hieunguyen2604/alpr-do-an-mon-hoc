...
| **NFR-P1 ĐẠT:** độ trễ E2E p95 = **731 ms** client-side / **780 ms** in-process trên `best.pt` (mục tiêu 800 ms) | Máy chạy **trong ngân sách**. Nếu bị hỏi "sao báo cáo đầu ghi 5.857 ms": con số cũ bị nhiễm do tải cạnh tranh + sai checkpoint + lỗi crop; đo lại trên máy rảnh với `best.pt` về 731 ms. Phân rã: OCR 60,8% / detect 38,0% |
