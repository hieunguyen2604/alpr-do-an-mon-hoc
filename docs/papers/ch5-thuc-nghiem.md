...
Phản xạ thông thường trước một máy không có GPU là coi đó là bất tiện nhất thời: "hiện chạy CPU, sau này có GPU thì nhanh hơn". Đồ án chủ ý **không** áp dụng cách nhìn đó, và điều này ảnh hưởng trực tiếp đến cách diễn giải Chương 5.

**Thứ nhất, môi trường vận hành đích là môi trường đã biết và không có GPU.** Chỉ tiêu NFR-P1 được phát biểu *kèm* ràng buộc CPU, chứ không phải phát biểu trong chân không rồi coi CPU là điều kiện ngoại cảnh bất lợi. Một hệ thống chỉ đạt chỉ tiêu khi có GPU là hệ thống *không đạt chỉ tiêu* trong bối cảnh sử dụng thật của nó. Do đó khi mục 5.7 kết luận NFR-P1 không đạt, kết luận ấy là **kết luận cuối cùng về hệ thống**, không phải kết luận tạm thời chờ nâng cấp phần cứng.

**Thứ hai, ràng buộc CPU thay đổi *lựa chọn mô hình*, không chỉ thay đổi *tốc độ chạy*.** Trên GPU, chênh lệch chi phí giữa YOLO11n và YOLO11m là không đáng kể; trên CPU, chênh lệch đó là hàng trăm mili-giây mỗi ảnh. Quyết định kiến trúc "suy luận trên CPU" vì thế kéo theo hai lựa chọn phái sinh đã cài đặt: biến thể **YOLO11n** (2.590.035 tham số) [17]<!-- jocher_2024_yolo11 --> cho tầng phát hiện, và bộ mô hình **PP-OCRv5 mobile** [23]<!-- paddlepaddle_2026_ppocrv5docs --> cho tầng nhận dạng ký tự. Đây là lựa chọn *do ràng buộc phần cứng quyết định*, và khi đánh giá kết quả phải đánh giá cả ràng buộc lẫn lựa chọn, không tách rời.

**Thứ ba, ràng buộc quyết định quy mô thực nghiệm.** Với khoảng 35,6 phút mỗi epoch trên CPU, một lượt huấn luyện 20 epoch mất khoảng 12 giờ liên tục. Điều này khiến **tìm kiếm siêu tham số trở nên bất khả thi trong khuôn khổ đồ án**, và giải thích vì sao chương này báo cáo *một* cấu hình huấn luyện chứ không phải một khảo sát siêu tham số. Đó là giới hạn thật của công trình và được ghi nhận trong mục 5.11.3, không được che giấu bằng cách trình bày cấu hình duy nhất ấy như thể nó là kết quả của một quá trình tối ưu.
...
| M6 | Tuân thủ lint và định dạng tự động | sạch | sạch | — | ⬜ | 5.9.2 |
| **NFR-S — An toàn** | | | | | | |
...
**Một cảnh báo hiệu lực về A7.** A7 = 0,5227 bị hạ thêm bởi giới hạn của giao thức đo: nó đo trên ảnh crop biển số (ngoài phân bố huấn luyện của bộ phát hiện), khiến tỉ lệ bỏ sót ở tầng phát hiện bị thổi phồng (11,96%). Trên ảnh hiện trường thật bộ phát hiện gần như không bỏ sót (mAP@0.5 = 0,9935). Do đó A7 nên đọc như **cận dưới bi quan**; đo A7 đúng cách cần một tập test hiện trường có nhãn chuỗi — việc chưa làm được (5.6.5).
