...
| NFR-P2 | Tốc độ khung hình chế độ webcam | ≥ 5 FPS hiệu dụng | ≥ 3 FPS |
...
| **(c) Phần mềm** | REST API bằng FastAPI có tài liệu Swagger tự sinh; nhận dạng từ ảnh, video và luồng webcam; lưu lịch sử bằng SQLite + SQLAlchemy + Alembic; giao diện web React + Vite + TypeScript + TailwindCSS; dashboard thống kê, lịch sử, tìm kiếm, lọc; đóng gói bằng Docker và Docker Compose |
...
Colab/Kaggle nằm **ngoài** ranh giới hệ thống khi vận hành — chúng chỉ là công cụ ngoại tuyến sản xuất ra tệp trọng số `best.pt`. Hệ thống khi chạy **không phụ thuộc vào bất kỳ dịch vụ ngoài nào**, đây là hệ quả trực tiếp của tiêu chí "demo chạy được không cần Internet" ở mục 1.2.3.
...
**Trạng thái tại thời điểm viết chương này.** Phase 0 và Phase 1 đã hoàn thành, chốt M0 và M1. Backend FastAPI đã chạy được và được xác minh bằng yêu cầu HTTP thật (10 endpoint phản hồi đúng, migration cơ sở dữ liệu hoàn tất, tài liệu Swagger render được); frontend đã hoàn thành và build sạch. **Hệ thống đang vận hành pipeline nhận dạng thật** với `models/baseline-416-v1.pt` — `/health` báo `model_loaded: true`, engine `yolo:baseline-416-v1.pt+paddleocr-PP-OCRv5-mobile`. Tuy nhiên **mô hình chính thức chưa huấn luyện xong**: baseline dùng `imgsz=416` trong khi chỉ tiêu đặt ở 640, và dùng split v1 vốn có rò rỉ train↔test, nên số của nó **không dùng làm kết quả đánh giá được**. Do đó **toàn bộ kết quả thực nghiệm sẽ được trình bày ở Chương 5** sau khi lượt huấn luyện `imgsz=640` trên split v3 kết thúc và các chỉ tiêu NFR-A4/A5/A6/A7 được đo.
