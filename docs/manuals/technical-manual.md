...
| 2 | **NFR-P1 không đạt** | Cao | 5.857 ms so với ngưỡng 1.500 ms. Bước xuất mô hình **đã làm xong** (6 bản ONNX/OpenVINO trong `models/checkpoints/exported/`) và đã đo: OpenVINO nhanh hơn PyTorch 4,7× khi chạy chung tiến trình với PaddleOCR. **Nhưng chưa dùng được** vì khiếm khuyết ở mục 13.10. Quyết định `AD-05` đã dự trù chính xác bước này ("chạy đúng trước, tối ưu sau"). ⚠ **Lưu ý định hướng:** đo đạc cho thấy **nút thắt nằm ở OCR, không ở bộ phát hiện** — hạ `imgsz` chỉ được ~6 ms. Tối ưu tiếp phải nhắm vào tầng OCR |
