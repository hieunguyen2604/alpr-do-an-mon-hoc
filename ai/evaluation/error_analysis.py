"""Group and export the OCR failures so they can be inspected by eye.

An aggregate accuracy figure says *how often* the system is wrong. It never
says *why*, and "why" is what the evaluation chapter of the thesis has to
answer. This script turns the per-crop records written by
:mod:`ai.evaluation.benchmark_ocr` into a diagnosis:

* every failure is placed in **one** error class -- character confusion, missing
  characters, extra characters, wrong reading order, empty read, or mixed;
* the failing crops are exported into one directory per class, optionally
  annotated with the true and the predicted string, so a whole class can be
  reviewed at a glance;
* the confusions, the failing positions and the failing plate lengths are
  tabulated, because the shape of an error distribution is what points at its
  cause.

Why the classes are these classes
---------------------------------
They map onto distinct root causes, which is the only thing that makes an error
taxonomy useful:

=================== ==================================================
Class               What it usually means
=================== ==================================================
``substitution``    The recogniser saw every character but read some
                    of them wrong -- a glyph problem. Repairable by the
                    positional rules, and the material from which the
                    correction tables should be rebuilt.
``missing_chars``   Characters were dropped: the crop was cut too
                    tight, or a two-line split amputated a row.
``extra_chars``     Characters were invented: plate border, bolt or
                    frame text read as a glyph.
``transposition``   Every character was read, in the wrong order. On a
                    two-line plate this is the signature of the halves
                    being merged in the wrong order (risk R-04).
``empty_read``      The engine returned nothing at all -- a detection
                    or image-quality problem, not an OCR one.
``mixed``           Several of the above at once, typically a badly
                    degraded crop.
=================== ==================================================

The ``transposition`` class earns its place despite being rare: it is the one
class that would be *invisible* in a character error rate, because a reordered
string can have the same character histogram as the truth.

This module reads the benchmark's JSON rather than re-running the engine. OCR is
the expensive half of the work, and re-running it would also mean the analysis
described a *different* set of predictions from the one the report published.

Example:
    python -m ai.evaluation.error_analysis \\
        --report docs/reports/04-ocr-benchmark/ocr_benchmark.json \\
        --output-dir docs/reports/04-ocr-benchmark/errors --annotate
"""
