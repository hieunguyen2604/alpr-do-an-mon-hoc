"""Detect train/test contamination in a YOLO dataset via perceptual hashing.

Why this script exists
----------------------
The detector reaches mAP@0.5 ~= 0.99 within a handful of epochs. There are only
two honest explanations:

1. The task is genuinely easy. Plate detection is a single-class,
   high-contrast, roughly-rectangular target; near-saturated mAP is plausible.
2. The split leaks. If the same photograph -- or a near-duplicate frame pulled
   from the same video burst -- appears in both ``train`` and ``test``, the
   model is being scored on images it memorised, and every downstream number in
   the thesis is invalid.

These two explanations are indistinguishable from the mAP value alone, so the
question has to be settled directly against the pixels.

Method
------
For every image in each split we compute a **perceptual hash** (``imagehash``'s
``phash``: DCT over a reduced-luminance image, 64-bit output). Unlike a
cryptographic digest, ``phash`` is stable under re-encoding, mild rescaling and
small compression changes, so it catches the duplicates that actually threaten
a split -- the same scene saved twice, or consecutive frames of one video --
which a byte-level checksum would miss entirely.

We then measure the **Hamming distance** between every cross-split pair and
count pairs at or below a threshold (default 5 of 64 bits). The
0-5 band is the conventional "visually the same image" region for a 64-bit
phash; 6-10 is "similar scene, arguably distinct"; above that is unrelated.

Interpreting the result
-----------------------
* **0 near-duplicate pairs** -- the split is clean and the high mAP reflects
  task difficulty, not memorisation.
* **> 0 pairs** -- a serious finding. The affected images must be reported and
  the dataset re-split, grouping by source scene rather than by file, before
  any accuracy number is quoted.

Cost note: the cross-split comparison is O(n_a * n_b), but each comparison is a
64-bit XOR plus popcount. We vectorise it by unpacking the hashes into a
``numpy`` bit matrix, which turns the whole thing into one matrix product per
split pair and keeps a 3203 x 458 comparison well under a second.

Example:
    python -m ai.evaluation.leak_check --dataset datasets/processed/yolo
"""
