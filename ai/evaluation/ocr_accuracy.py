"""Measure NFR-A4 to NFR-A8: OCR and end-to-end plate-reading accuracy.

This is the Phase 4 accuracy measurement. :mod:`ai.evaluation.benchmark_ocr`
already measures the *cropped-plate* half of the question and owns the string
metrics; this module reuses those metrics unchanged and adds the three things
the requirement set asks for that the benchmark does not cover:

1. an **end-to-end** pass (NFR-A7) that starts from an image file and runs
   detection, cropping, recognition and normalisation, rather than being handed
   a crop;
2. the **aspect-ratio repair** the label corpus requires -- see
   :ref:`the caveat below <ocr-accuracy-caveats>`;
3. a single JSON payload keyed by requirement id, so the report can state pass
   or fail per requirement instead of leaving a reader to derive it.

What each requirement is measured as
------------------------------------
======== ================================================================
Id       Measured quantity
======== ================================================================
NFR-A4   ``1 - CER``, character error rate over the whole corpus:
         ``sum(edit distance) / sum(len(truth))``. Reported both before
         and after normalisation; the headline is the *delivered* string,
         which is the normalised one.
NFR-A5   Exact full-string match rate using ``raw_ocr_text``, i.e. the
         engine output with only case folding and separator stripping
         applied (:func:`~ai.inference.plate_rules.clean_text`).
NFR-A6   Exact full-string match rate using ``plate_number``, the output
         of :class:`~ai.inference.normalizer.VietnamesePlateNormalizer`.
NFR-A7   Exact full-string match rate of the end-to-end pipeline: image
         file in, plate string out. A plate the detector misses counts as
         a failure, which is the whole point of measuring it separately.
NFR-A8   Every figure above, reported again split by one-line and
         two-line plates.
======== ================================================================

**NFR-A6 minus NFR-A5 is the quantitative contribution of the post-processing
block.** It is reported as its own field, ``postprocessing_gain_points``, and
it is the reason ``raw_ocr_text`` exists as a column in ``detection_history``
at all. A negative value is reported as a negative value.

Corpus-level versus per-sample CER
----------------------------------
The character error rate is aggregated **corpus-level** -- total edit distance
over total reference length -- not as the mean of per-sample rates. The two
differ, and the per-sample mean is the wrong one here: it weights a 7-character
plate the same as a 9-character plate, and a single catastrophic sample whose
rate exceeds 1.0 can move it more than the arithmetic justifies. The per-sample
mean is reported alongside, labelled as such, because
:mod:`ai.evaluation.benchmark_ocr` publishes that variant.

.. _ocr-accuracy-caveats:

Three caveats that must travel with every number this module produces
---------------------------------------------------------------------
**1. The label corpus is aspect-ratio destroyed.** Both Roboflow OCR datasets
export their plate crops resized to a *square* canvas (640x640 and 416x416),
which discards the single most informative geometric cue a plate crop carries.
Two consequences follow, and both are handled explicitly rather than ignored:

* :func:`~ai.inference.two_line.estimate_line_count` cannot work on a square
  image -- every crop reads as aspect ratio 1.0 and is therefore classified
  two-line. The evaluation restores a canonical aspect ratio from the label's
  ``line_count`` before recognition (:data:`CANONICAL_ASPECT_RATIO`, taken from
  the physical plate sizes in QCVN 08:2024/BCA). This is an **oracle step**: it
  uses ground truth the deployed system does not have. It is applied because
  the alternative measures a dataset export setting, not the recogniser. The
  un-repaired figure is measured too and reported as
  ``aspect_ratio_repair_ablation`` so the size of the effect is visible.
* Upscaling a small crop onto a 640x640 canvas makes the glyphs far larger than
  any text the PP-OCR detector was trained on. See the next caveat.

**2. The end-to-end figure does not start from a traffic scene.** No dataset in
this project carries both a full scene *and* a plate transcription: the seven
detection datasets have boxes without text, and the two OCR datasets have text
without scenes. NFR-A7 is therefore measured on the OCR crops, and the detector
is asked to find a plate that fills the whole frame. That is **easier** than the
deployed task in framing and **harder** in resolution and aspect. The figure is
a bound on the recognition half of the pipeline under a detector that has to
re-localise, not a substitute for a scene-level measurement, and it is labelled
``e2e_on_plate_crops`` for that reason.

**3. The end-to-end figure is, however, leak-free.** ``roboflow_ocr_plate`` and
``roboflow_ocr_conversion`` appear in neither ``merged_v2`` nor any YOLO split
-- the detector's training corpus is the other seven datasets. The detector has
not seen these images. (Their *source photographs* could still overlap another
dataset; that cannot be checked with a perceptual hash, because a tight crop
and the scene it came from do not hash alike.)
