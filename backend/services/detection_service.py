...
class DetectionService:
    ...
    def _create_job(
        ...
        *,
        ...
        Returns:
            The persisted (not yet committed) job.
        """
        job = DetectionJob(
        ...
        db.add(job)
        # Flush, not commit: the id is generated by the default and is needed to
        # name the plate crops, but the transaction must stay open so that a
        # later failure rolls the whole detection back rather than leaving a
        # job with no results.
        db.flush()
        return job
