...
- **Kiểm thử** — **1.004/1.004 test pass, 0 fail, 0 `xfail`** (chạy 13/08/2026 bằng `backend/.venv/Scripts/python.exe -m pytest -q` từ gốc kho); bao phủ tầng nghiệp vụ **87,7%** theo lần đo 20/07/2026 ([13-refactor-result.json](docs/reports/13-refactor-result.json)) và **88,1%** theo lần đo Phase 7 trước đó ([07-testing-report.md](docs/reports/07-testing-report.md)) — NFR-M2 ≥ 70% ⇒ đạt ở cả hai mốc; 18/18 test kiến trúc pass.
- **Benchmark (Phase 7)** — đo trên **Intel i5-14600K, 14 nhân vật lý / 20 luồng logic, 31,77 GiB RAM, Windows 11, Python 3.13.12, torch 2.13.0+cpu, CPU-only**. ✅ **NFR-P1 = 509,76 ms p95** ở cấu hình giao hàng — dưới cả mục tiêu 800 ms; trung vị chỉ 150,07 ms. Cặp 731,15 / 780,36 ms là lần đo **20/07**, trước khi có bậc thang thử-lại. Phân rã bước: OCR **64,3%** (~112,55 ms/biển), detect **34,2%** (~59,83 ms). Truy vấn CSDL 10.000 bản ghi **18,71 ms** (chỉ tiêu 500 ms), overhead API **19,01 ms** (50 ms), RAM **0,88 GB** (2 GB), khởi động **8,36 s** (15 s) *(P4–P7 đo trên baseline)*, 0 lỗi ở mức đồng thời 10, soak 15 phút 100% trên 5.337 yêu cầu. **NFR-P2/P3/R4/R5 đã đo 02/08/2026** ([33-runtime-nfr.json](docs/reports/33-runtime-nfr.json)): ✅ **P2 = 5,63 FPS** (đo lại 13/08 trên máy rảnh; vượt cả mục tiêu 5. Con số 2,379 công bố 02/08 đo khi máy đang cõng ~560% CPU của tiến trình khác và harness đã tự dán nhãn *bi quan* — xem [38](docs/reports/38-runtime-backend-and-nfr-p2.md)); ✅ P3 = 0,8695× · ✅ R4 = 100% trên 5.337 yêu cầu · ✅ R5 = 0/7.977 bản ghi mất. Con số cũ 5.857 ms (epoch 7, máy bận, lỗi crop) **đã bị bác bỏ** — xem [07-benchmark-p1-resolved.json](docs/reports/07-benchmark-p1-resolved.json). Chi tiết: [07-benchmark-report.md](docs/reports/07-benchmark-report.md).
- **Docker** — dựng lại và kiểm chứng **14/08/2026** trên đúng mã đang giao: `alpr-backend` 3,81 GB + `alpr-frontend` 93,2 MB, stack `Healthy`, container nạp `/app/models/best.pt`, mount `:ro` có hiệu lực, nhận dạng thật qua HTTP. Độ trễ đầu-cuối p95 **319 ms** trên 30 ảnh test — **đóng gói container không thêm chi phí đáng kể** (đây không phải con số NFR-P1 mới; xem ghi chú ở mục 6.1). 4 lỗi thật đã phát hiện và sửa ở lượt Phase 8. Chi tiết: [08-deployment-guide.md](docs/reports/08-deployment-guide.md).
...
7. **🔴 Nút thắt đã dịch sang độ chính xác OCR biển 2 dòng — đây là món nợ kỹ thuật lớn nhất còn lại.** Đo trên 2.801 biển: **NFR-A4 = 0,9483** (sàn 0,92 ✅, mục tiêu 0,95) · **NFR-A5 = 0,6373** (0,85) · **NFR-A6 = 0,7701** (0,90) — **A4 đạt sàn, A5 và A6 chưa**. Nhưng toàn bộ khoảng cách nằm ở **biển 2 dòng**: biển 1 dòng đạt **0,9925 / 0,9418 / 0,9541**, tức **vượt cả ba mục tiêu**; biển 2 dòng chỉ **0,9380 / 0,5600 / 0,7234** — chênh **23,07 điểm** ở mức chuỗi. Đóng góp của khối hậu xử lý là **+13,28 điểm** (372 biển sửa đúng, 0 biển hỏng ở mức chuỗi) — nhưng ở **mức ký tự** thì bảng ánh xạ **có chỗ sai**: hai quy tắc `L → 1` và `7 → T` kích hoạt đúng chỗ nhưng **ánh xạ tới ký tự sai** (phải là `L → 4` và `7 → Z`), khiến lỗi bị **dời chỗ** chứ không bị xoá. Chi tiết: [04-ocr-report.md](docs/reports/04-ocr-report.md) mục 6.1b.
...
**Đã đo bổ sung trong đợt này:** NFR-A4/A5/A6/A7 (mục 7 ở trên), **NFR-P2 = 5,63 FPS** (✅ vượt cả mục tiêu 5; xấu nhất đo được dưới tải nặng **4,057**, vẫn trên OpenVINO@640), **NFR-P3 = 0,8695×** ✅ (mục tiêu 0,3×; nguồn [`37-nfr-p2-pytorch.json`](docs/reports/37-nfr-p2-pytorch.json) — số 0,754× của vòng Phase 7 và 0,746× của lượt 02/08 đều đã được thay thế), **NFR-R5 = 0 bản ghi mất** ✅ (4.470 → 4.470, tắt bằng `TerminateProcess` chứ không tắt êm). **NFR-R4** đã nâng từ 5 lên **15 phút / 5.337 request / 0 lỗi**, nhưng đặc tả yêu cầu **60 phút** nên vẫn chỉ là bằng chứng một phần.
