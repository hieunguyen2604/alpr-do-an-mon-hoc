...
- **Chạy hoàn toàn không cần GPU** khi vận hành.
...
1. **✅ Mô hình chính thức `best.pt` đã có và đang chạy.** Lượt chính thức đã **huấn luyện xong** (`models/best.pt`, YOLO11n, `imgsz=640`, split v3, 20 epoch): mAP@0.5 = **0,9829**, mAP@0.5:0.95 = **0,7834**, P = **0,9837**, R = **0,9714** — **đạt cả bốn**, đo trên split v3 khử trùng lặp (không còn rò rỉ tên-tệp thổi phồng). Backend nạp chính mô hình này — `GET /health` trả **`model_loaded: true`**, `engine = 'yolo:best.pt+paddleocr-PP-OCRv5-mobile'`. `baseline-416-v1.pt` (40 epoch, `imgsz=416`, split v1, mAP@0.5 = 0,9933) giờ là **mô hình đối chứng** — ⚠️ **không được báo cáo là "đạt"** vì hai khiếm khuyết: (a) `imgsz` = 416 ≠ 640; (b) split v1 **có rò rỉ train↔test** (xem giới hạn 5). Xem [models/README.md](models/README.md).
...
7. **🔴 Nút thắt đã dịch sang độ chính xác OCR biển 2 dòng — đây là món nợ kỹ thuật lớn nhất còn lại.** Đo trên 2.801 biển: **NFR-A4 = 0,8734** (mục tiêu 0,95) · **NFR-A5 = 0,6098** (0,85) · **NFR-A6 = 0,6555** (0,90) — **trượt cả ba**. Nhưng toàn bộ khoảng cách nằm ở **biển 2 dòng**: biển 1 dòng đạt **0,9900 / 0,9418 / 0,9489**, tức **vượt cả ba mục tiêu**; biển 2 dòng chỉ **0,8462 / 0,5255 / 0,5810** — chênh **36,8 điểm**. Đóng góp của khối hậu xử lý là **+4,57 điểm** (128 biển sửa đúng, 0 biển hỏng ở mức chuỗi) — nhưng ở **mức ký tự** thì bảng ánh xạ **có chỗ sai**: hai quy tắc `L → 1` và `7 → T` kích hoạt đúng chỗ nhưng **ánh xạ tới ký tự sai** (phải là `L → 4` và `7 → Z`), khiến lỗi bị **dời chỗ** chứ không bị xoá. Chi tiết: [04-ocr-report.md](docs/reports/04-ocr-report.md) mục 6.1b.
...
| [04-ocr-report.md](docs/reports/04-ocr-report.md) | Thiết kế khối OCR và cách đo NFR-A4/A5/A6 — ⚠️ **bảng kết quả còn trống, chưa đo** |
...
- **Mã AI không được import FastAPI** — ràng buộc kiến trúc bắt buộc, kiểm tra tự động ở Phase 7.
